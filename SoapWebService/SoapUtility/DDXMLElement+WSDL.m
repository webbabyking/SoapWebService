...
+(DDXMLElement *)LoadWSDLFromXMLString:(NSString *)xmlstring{
...
-(NSString *)TargetNamespace{
    return [[self attributeForName:@"targetNamespace"] stringValue];
}
...
-(NSArray *)GetMethodParamsByMethodName:(NSString *)methodName{
    ...
    if(paramElements==nil)
    {
        paramElements=[self Message_Parts:methodName];
    }
...
-(DDXMLElement *)SOAPBinding:(SOAPTYPE)SoapType{
    ...
    for (DDXMLElement *binding in bindings) {
        ...
        soapTest = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", soapRegex];
        result= [soapTest evaluateWithObject:attrnameValue];
        if(result){
            return binding;
        }
...
//definitinos->binding(soapBinding/soap12Binding)->operation 有多个operation
...
-(DDXMLElement *)Message_Name:(NSString *)methodName{
    ...
    if(messages){
        for (DDXMLElement *message in [self Messages]) {
            ...
            messageName=[NSString stringWithFormat:@"%@%@",methodName,@"Request"];
            if([attrnameValue isEqualToString:messageName]){
                return message;
            }
        }
...
-(NSArray *)Message_Parts:(NSString *)methodName{
    DDXMLElement *message=[self Message_Name:methodName];
    if(message){
        return [self GetChildrenNodeByNodeName:@"part" ParentNode:message];
    }else{
        return nil;
    }
}
